...
export default function removeFromStage(
  entity: GameEntity,
  gameStage: GameStage | FiskGame
) {
  const stage = isEngine(gameStage)
    ? (gameStage.currentStage as GameStage)
    : gameStage;
  ...
  const interactorsIndex = stage.interactors.indexOf(entity as Interactable);
